...
struct StmtRange : std::pair<StmtIterator, StmtIterator> {
  ...
  StmtRange(Stmt::child_range range)
      : std::pair<StmtIterator, StmtIterator>(range.begin(), range.end()) {}
};
#endif
 ...
 */
struct less_name {
  bool operator()(const clang::TypeDecl* x, const clang::TypeDecl* y) {
...
struct PetTypes {
  ...
  void insert(clang::RecordDecl* decl) { records.insert(decl); }
  void insert(clang::TypedefNameDecl* decl) { typedefs.insert(decl); }
};
...
struct PetScan {
  ...
  PetScan(clang::Preprocessor& PP, clang::ASTContext& ast_context,
          clang::DeclContext* decl_context, ScopLoc& loc, pet_options* options,
  ...
  struct pet_array* extract_array(__isl_keep isl_id* id, PetTypes* types,
 ...
 private:
  ...
  struct pet_scop* scan_arrays(struct pet_scop* scop,
                               __isl_keep pet_context* pc);
  struct pet_array* extract_array(clang::ValueDecl* decl, PetTypes* types,
                                  __isl_keep pet_context* pc);
  ...
  __isl_give pet_tree* insert_initial_declarations(__isl_take pet_tree* tree,
                                                   int n_decl,
  ...
  __isl_give pet_tree* update_loc(__isl_take pet_tree* tree, clang::Stmt* stmt);
  ...
  __isl_give pet_expr* extract_unary_increment(clang::UnaryOperator* op,
                                               clang::ValueDecl* iv);
  __isl_give pet_expr* extract_binary_increment(clang::BinaryOperator* op,
                                                clang::ValueDecl* iv);
  __isl_give pet_expr* extract_compound_increment(
      clang::CompoundAssignOperator* op, clang::ValueDecl* iv);
  ...
  __isl_give pet_expr* extract_index_expr(clang::ArraySubscriptExpr* expr);
